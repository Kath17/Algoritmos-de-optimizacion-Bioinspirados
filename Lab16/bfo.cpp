...
#define S       4      /* population size */
#define Sr      S/2     /* number to split */
#define ss      0.1     /* step size */
#define N_ed    2       /* number of elimination-dispersal events */
#define N_re    2       /* number of reproduction steps */
#define N_ch    20      /* number of chemotactic steps */
#define N_sl    5       /* swim length */
...
void objective_function(Cell *x)
{
    ...
    rez = -cos(x->vect[0])*cos(x->vect[1])*exp(-pow((x->vect[0]-PI),2)-pow((x->vect[1]-PI),2));

    x->cost = rez;

    if(x->cost < best)
        best = x->cost;
        best_cell[0] = x->vect[0];
        best_cell[1] = x->vect[1];
